...
static int parse_bdaddr(const char *s, struct bdaddr *out)
{
    unsigned int a[6];
...
static void hexdump_prefix(const char *prefix, const uint8_t *buf, size_t len, size_t max)
{
...
static int set_exact_connect_filter(int fd)
{
    // Read current filter first (hcitool does this — required for kernel init)
    uint8_t dummy[16];
    socklen_t len = sizeof(dummy);
    getsockopt(fd, SOL_HCI, HCI_FILTER, dummy, &len);
    ...
    if (setsockopt(fd, SOL_HCI, HCI_FILTER, hci_filter, sizeof(hci_filter)) < 0) {
        perror("setsockopt exact HCI_FILTER");
        return -1;
    }
    return 0;
}
...
static int hci_send_cmd(int fd, uint16_t ogf, uint16_t ocf,
...
static int hci_connect_le(int fd, const struct bdaddr *peer, uint16_t *handle)
{
    // Exact params from hcitool strace
    uint8_t p[25];
    memset(p, 0, sizeof(p));
    put_le16(p + 0, 0x0004);   // scan_interval (from strace)
    put_le16(p + 2, 0x0004);   // scan_window (from strace)
    p[4] = 0x00;               // filter_policy
    p[5] = 0x00;               // peer_addr_type (public)
    memcpy(p + 6, peer->b, 6);
    p[12] = 0x00;              // own_addr_type
    put_le16(p + 13, 0x000F);  // conn_interval_min (from strace)
    put_le16(p + 15, 0x000F);  // conn_interval_max (from strace)
    put_le16(p + 17, 0x0000);  // latency
    put_le16(p + 19, 0x0C80);  // supervision_timeout (from strace: 3200*10ms)
    put_le16(p + 21, 0x0001);  // min_ce_length (from strace)
    put_le16(p + 23, 0x0001);  // max_ce_length (from strace)

    if (set_exact_connect_filter(fd) < 0)
        return -1;
    if (hci_send_cmd(fd, OGF_LE_CTL, OCF_LE_CREATE_CONN, p, sizeof(p)) < 0)
        return -1;

    fprintf(stderr, "waiting for LE connection complete (using writev + exact strace params)\n");
    for (int i = 0; running && i < 120; i++) {
        uint8_t pkt[260];
        int n = hci_read_packet(fd, pkt, sizeof(pkt), 100);
        if (n <= 0) {
            if (n < 0) fprintf(stderr, "  read err: %s\n", strerror(errno));
            continue;
        }
        fprintf(stderr, "  pkt type=%02x len=%d: ", pkt[0], n);
        for (int j = 1; j < n && j < 8; j++) fprintf(stderr, "%02x ", pkt[j]);
        fprintf(stderr, "\n");
        ...
        if (evt == EVT_CMD_STATUS && plen >= 4) {
            fprintf(stderr, "  CMD_STATUS: opcode=0x%04x status=0x%02x\n",
                    get_le16(ev), ev[2]);
            if (ev[0] != 0x00 || ev[1] != 0x20 || ev[2] != 0x00)
                fprintf(stderr, "  unexpected CMD_STATUS\n");
        } else if (evt == EVT_LE_META && plen >= 19 && ev[0] == EVT_LE_CONN_COMPLETE) {
            uint8_t status = ev[1];
            uint16_t raw_handle = get_le16(ev + 2);
            if (status != 0x00) {
                fprintf(stderr, "LE connection failed status=0x%02x\n", status);
                return -1;
            }
            *handle = (uint16_t)(raw_handle & 0x0fff);
            fprintf(stderr, "connected: handle=0x%04x\n", *handle);
            return 0;
        }
    }

    fprintf(stderr, "timed out waiting for LE connection complete\n");
    return -1;
}
...
static void hci_request_conn_update(int fd, uint16_t handle)
{
    ...
    if (hci_send_cmd(fd, OGF_LE_CTL, OCF_LE_CONN_UPDATE, p, sizeof(p)) == 0)
        fprintf(stderr, "requested LE connection update: 7.5ms interval\n");
}

static int acl_send_att(int fd, uint16_t handle, const uint8_t *att, uint16_t att_len)
{
...
static int read_acl_att(int fd, uint16_t conn_handle, uint8_t *att, size_t attsz,
                        uint16_t *out_handle, int timeout_ms)
{
    uint8_t pkt[2048];
    ...
    for (;;) {
        ...
        if (pkt[0] == HCI_EVENT_PKT && n >= 3) {
            ...
            if (evt == EVT_LE_META && n >= 6 && ev[0] == EVT_LE_CONN_UPDATE_COMPLETE) {
...
static void gatt_probe(int fd, uint16_t handle)
{
    ...
    uint8_t svc_req[7] = {ATT_READ_BY_GROUP_REQ, 0x01, 0x00, 0xff, 0xff, 0x00, 0x28};
    ...
    uint8_t chr_req[7] = {ATT_READ_BY_TYPE_REQ, 0x08, 0x00, 0x2a, 0x00, 0x03, 0x28};
...
static int report09_to_uhid(const uint8_t *r, size_t len, uint8_t *out, size_t outsz)
{
...
static int usb_init_controller(const char *hidraw)
{
    ...
    if (usb_send_subcmd(fd, 0, 0x06, &one, 1) < 0 ||
        usb_send_subcmd(fd, 1, 0x08, &zero, 1) < 0 ||
        usb_send_subcmd(fd, 2, 0x01, host, sizeof(host)) < 0) {
        close(fd);
        return -1;
    }
...
{
    fprintf(stderr, "usage: %s [bdaddr] [--usb-init] [--no-uhid] [--hidraw PATH]\n", argv0);
}

static int run_session(const struct bdaddr *peer, int no_uhid)
{
    int hci = -1;
    int uhid = -1;
    uint16_t handle = 0;
    ...
    if (hci_connect_le(hci, peer, &handle) < 0) {
        close(hci);
        return -1;
    }

    if (set_acl_runtime_filter(hci) < 0) {
        close(hci);
        return -1;
    }

    hci_request_conn_update(hci, handle);
    gatt_probe(hci, handle);
    ...
    if (enable_notifications(hci, handle) < 0) {
    ...
    while (running) {
        int n = read_acl_att(hci, handle, att, sizeof(att), NULL, 1000);
        if (n == -2)
            break;
        if (n <= 0)
            continue;
        if (n >= 3 && att[0] == ATT_HANDLE_NOTIFY) {
            ...
            if (value_handle != SW2_INPUT_VALUE_HANDLE)
                continue;
            int hlen = report09_to_uhid(report, report_len, hid, sizeof(hid));
            if (hlen > 0)
...
int main(int argc, char **argv)
{
    ...
    if (parse_bdaddr(bdaddr_s, &peer) < 0) {
    ...
    fprintf(stderr, "sw2d: target=%s hci=hci0 uhid=%s\n",
