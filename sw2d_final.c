...
/* LE Connection Parameter Update (inline in main loop) */
...
parse_pairing_request: {
    ...
    int sc_len = smp_read_pdu(ctx, sconfirm_pdu, sizeof(sconfirm_pdu), 5000);
...
static int att_read_rsp(rx_ctx_t *ctx, uint8_t *buf, int maxlen, int timeout_ms) {
    ...
    while (g_running) {
        ...
        if (remaining > 500) remaining = 500;  /* cap poll interval */
        if (remaining < 10)  remaining = 10;   /* floor */
...
int main(int argc, char **argv) {
    ...
    while (g_running) {
        ...
        /* Consume MTU response before GATT discovery */
        {
            uint8_t mtu_rsp[8];
            int mtu_len = att_read_rsp(&ctx, mtu_rsp, sizeof(mtu_rsp), 2000);
            if (mtu_len >= 3 && mtu_rsp[0] == ATT_OP_EXCHANGE_MTU_RSP) {
                uint16_t server_rx_mtu = get_le16(mtu_rsp + 1);
                fprintf(stderr, "  ATT MTU: server_rx=%u\n", server_rx_mtu);
            } else {
                fprintf(stderr, "  Note: no MTU response (got %d bytes op=0x%02x) — continuing\n",
                        mtu_len, mtu_len > 0 ? mtu_rsp[0] : 0);
            }
        }
